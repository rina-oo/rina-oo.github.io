...
## 표(Table)와 그림(Figure) — 필수 규칙

- **표는 논문 형식 그대로 옮긴다.** 임의로 행을 추리거나 요약하지 말고, 논문의 표를 마크다운 표로 전체 재현한다(볼드=1위 표기 등 서식 포함). 표 아래에 해석 문단을 덧붙인다.
- **Figure는 캡처 이미지로 포함하고, 반드시 해석·설명을 함께 쓴다.**
  - `python3 scripts/extract_figures.py <pdf> assets/img/posts/<arxiv_id> <figure번호들>` 로 PDF에서 캡처한다 (컨테이너에 `pymupdf` 필요).
  - 이미지는 `assets/img/posts/<arxiv_id>/figureN.png`에 저장하고 본문에서 `![...](/assets/img/posts/<arxiv_id>/figureN.png)` + 이탤릭 캡션으로 삽입한다.
  - 이미지 바로 아래에 "이 그림이 무엇을 보여주고, 왜 중요한가"를 설명하는 문단을 반드시 넣는다. 캡처만 붙이고 지나가지 않는다.
  - 최소 포함 대상: 아키텍처 개요도, 핵심 결과 그래프. 그 외 논지 전개에 필요한 그림.
